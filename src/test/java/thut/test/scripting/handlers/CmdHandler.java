...
public class CmdHandler implements ICmdHandler
{
    private static class SourceWrapper extends CommandSourceStack
    {
        ...
        {
            super(wrapped.getServer(), wrapped.getPosition(), wrapped.getRotation(), wrapped.getLevel(), 4,
                    wrapped.getTextName(), wrapped.getDisplayName(), wrapped.getServer(), wrapped.getEntity());
        }
    ...
    public String handle(final MinecraftServer server, final String input)
    {
        ...
        server.getCommands().performPrefixedCommand(src, input);
        return src.lastMsg.getString();
    }
