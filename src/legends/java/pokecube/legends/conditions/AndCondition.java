...
public class AndCondition extends AbstractCondition
{
    ...
    public AndCondition(AbstractCondition A, AbstractCondition B)
    {
        this.A = A;
        this.B = B;
        if (A.getEntry() != B.getEntry())
        {
            PokecubeAPI.LOGGER.error("Warning, two conditions handed in with different entries to an AND condition!");
        }
    }
