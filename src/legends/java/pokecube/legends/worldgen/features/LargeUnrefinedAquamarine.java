...
public class LargeUnrefinedAquamarine extends Feature<LargeDripstoneConfiguration>
{
    ...
    public boolean place(FeaturePlaceContext<LargeDripstoneConfiguration> context)
    {
        ...
        {
            Optional<Column> optional = Column.scan(world, pos,
                    largeUnrefinedAquamarineConfig.floorToCeilingSearchRange, AquamarineUtils::isEmptyOrWater,
            ...
            if (optional.isPresent() && optional.get() instanceof Column.Range)
            {
                ...
                else
                {
                    ...
                    int k = Mth.randomBetweenInclusive(random,
                            largeUnrefinedAquamarineConfig.columnRadius.getMinValue(), j);
                    LargeUnrefinedAquamarine.LargeUnrefineAquamarine largeUnrefinedAquamarine = makeAquamarine(
                            pos.atY(column.ceiling() - 1), false, random, k,
                            largeUnrefinedAquamarineConfig.stalactiteBluntness,
                            largeUnrefinedAquamarineConfig.heightScale);

                    LargeUnrefinedAquamarine.LargeUnrefineAquamarine largeUnrefinedAquamarine1 = makeAquamarine(
                            pos.atY(column.floor() + 1), true, random, k,
                            largeUnrefinedAquamarineConfig.stalagmiteBluntness,
                            largeUnrefinedAquamarineConfig.heightScale);
                    ...
                    boolean flag = largeUnrefinedAquamarine.moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(
                            world, windOffsetter);
                    boolean flag1 = largeUnrefinedAquamarine1.moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(
                            world, windOffsetter);
                    if (flag)
                    {
    ...
    public static LargeUnrefinedAquamarine.LargeUnrefineAquamarine makeAquamarine(BlockPos pos, boolean b,
            RandomSource random, int i, FloatProvider floatProvider, FloatProvider floatProvider1)
    {
        return new LargeUnrefinedAquamarine.LargeUnrefineAquamarine(pos, b, i, floatProvider.sample(random),
                floatProvider1.sample(random));
    }

    public void placeDebugMarkers(WorldGenLevel world, BlockPos pos, Column.Range column,
            LargeUnrefinedAquamarine.WindOffsetter windOffsetter)
    {
        world.setBlock(windOffsetter.offset(pos.atY(column.ceiling() - 1)), Blocks.DIAMOND_BLOCK.defaultBlockState(),
                2);
        world.setBlock(windOffsetter.offset(pos.atY(column.floor() + 1)), Blocks.GOLD_BLOCK.defaultBlockState(), 2);

        for (BlockPos.MutableBlockPos mutablePos = pos.atY(column.floor() + 2).mutable();
                mutablePos.getY() < column.ceiling() - 1; mutablePos.move(Direction.UP))
        {
            BlockPos pos1 = windOffsetter.offset(mutablePos);
            if (AquamarineUtils.isEmptyOrWater(world, pos1) || world.getBlockState(pos1)
                    .is(BlockInit.UNREFINED_AQUAMARINE.get()))
            {
    ...
    static final class LargeUnrefineAquamarine
    {
        ...
        boolean moveBackUntilBaseIsInsideStoneAndShrinkRadiusIfNecessary(WorldGenLevel world,
                LargeUnrefinedAquamarine.WindOffsetter windOffsetter)
        {
            while (this.radius > 1)
            {
                ...
                for (int j = 0; j < i; ++j)
                {
                    ...
                    if (AquamarineUtils.isCircleMostlyEmbeddedInStone(world, windOffsetter.offset(mutablePos),
                            this.radius))
                    {
                ...
                for (int j = -this.radius; j <= this.radius; ++j)
                {
                    ...
                    if (!(f > this.radius))
                    {
                        ...
                        if (k > 0)
                        {
                            ...
                            boolean flag = false;
                            int l = this.pointingUp ? world.getHeight(Heightmap.Types.WORLD_SURFACE_WG,
                                    mutablePos.getX(), mutablePos.getZ()) : Integer.MAX_VALUE;
