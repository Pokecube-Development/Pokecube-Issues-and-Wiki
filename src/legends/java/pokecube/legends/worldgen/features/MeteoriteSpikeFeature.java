...
public class MeteoriteSpikeFeature extends Feature<NoneFeatureConfiguration>
{
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context)
    {
        ...
        WorldGenLevel world;
        for (world = context.level();
                world.isEmptyBlock(pos) && pos.getY() > world.getMinBuildHeight() + 2; pos = pos.below())
        {
        }
                ...
                for (int i1 = -l; i1 <= l; ++i1)
                {
                    ...
                    for (int j1 = -l; j1 <= l; ++j1)
                    {
                                ...
                                i1 != -l && i1 != l && j1 != -l && j1 != l || !(random.nextFloat() > 0.75F)))
                        {
                            ...
                            if (stateOffset.isAir() || isDirt(stateOffset) || isSand(stateOffset) || isSandstone(
                                    stateOffset) || isStone(stateOffset) || stateOffset.is(BlockInit.ASH_BLOCK.get())
                                    || stateOffset.is(Blocks.ICE) && !noPlacement(state))
                            {
                                this.setBlock(world, pos.offset(i1, k, j1),
                            ...
                            if (k != 0 && l > 1)
                            {
                                ...
                                if (stateOffset.isAir() || isDirt(stateOffset) || isSand(stateOffset) || isSandstone(
                                        stateOffset) || isStone(stateOffset) || stateOffset.is(
                                        BlockInit.ASH_BLOCK.get()) || stateOffset.is(Blocks.ICE) && !noPlacement(state))
                                {
                                    this.setBlock(world, pos.offset(i1, -k, j1),
                                            BlockInit.METEORITE_BLOCK.get().defaultBlockState());
                                }
            ...
            for (int l1 = -k1; l1 <= k1; ++l1)
            {
                for (int i2 = -k1; i2 <= k1; ++i2)
                {
                    ...
                    while (posOffset.getY() > 50)
                    {
                        ...
                        if (!stateOffset.isAir() && !isDirt(stateOffset) && !isSand(stateOffset) && !stateOffset.is(
                                BlockInit.ASH.get()) && !stateOffset.is(BlockInit.ASH_BLOCK.get()) && !stateOffset.is(
                                Blocks.ICE) && !stateOffset.is(BlockInit.METEORITE_BLOCK.get()))
                        {
                            break;
                        }
