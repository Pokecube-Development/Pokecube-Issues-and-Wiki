...
public class PokemobsDatabases
{
    ...
    public static void load()
    {
                ...
                try
                {
                    ...
                    if (database != null)
                    {
                        ...
                        if (!database.requiredMods.isEmpty())
                        {
                            ...
                            for (final String s : database.requiredMods) allHere = allHere || ModList.get().isLoaded(s);
                            if (!allHere) return;
                        }
                        if (PokecubeCore.getConfig().debug_data)
                            PokecubeAPI.logInfo("Loaded Database File: {}, entries: {}", l, database.pokemon.size());
        ...
        for (final PokemobsJson json : allFound)
            for (final XMLPokedexEntry e : json.pokemon) if (PokemobsDatabases.compound.__map__.containsKey(e.name))
        {
