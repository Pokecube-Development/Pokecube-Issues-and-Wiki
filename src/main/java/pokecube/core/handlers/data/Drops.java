...
public class Drops extends LootTableProvider
{
    ...
    // {
    // private static final LootItemCondition.Builder SILK_TOUCH =
    // MatchTool.toolMatches(
    // ItemPredicate.Builder.item().hasEnchantment(new
    // EnchantmentPredicate(Enchantments.SILK_TOUCH,
    // MinMaxBounds.Ints.atLeast(1))));
    // private static final LootItemCondition.Builder NO_SILK_TOUCH =
    // BlockLoot.SILK_TOUCH.invert();
    // private static final LootItemCondition.Builder SHEARS =
    // MatchTool.toolMatches(
    // ItemPredicate.Builder.item().of(Items.SHEARS));
    // private static final LootItemCondition.Builder SILK_TOUCH_OR_SHEARS =
    // BlockLoot.SHEARS.or(
    // BlockLoot.SILK_TOUCH);
    // private static final LootItemCondition.Builder NOT_SILK_TOUCH_OR_SHEARS =
    // BlockLoot.SILK_TOUCH_OR_SHEARS
    // .invert();
    // private static final float[] DEFAULT_SAPLING_DROP_RATES = new float[] {
    // 0.05F, 0.0625F,
    // 0.083333336F, 0.1F };
    //
    // /**
    ...
    // */
    // protected static LootTable.Builder droppingWithChancesAndDrop(final Block
    // blockA, final ItemLike dropA,
    // final ItemLike dropB, final float... chances)
    // {
    // return BlockLoot.createSilkTouchOrShearsDispatchTable(blockA,
    // BlockLoot.applyExplosionCondition(blockA,
    // LootItem.lootTableItem(dropA)).when(BonusLevelTableCondition.bonusLevelFlatChance(Enchantments.BLOCK_FORTUNE,
    // chances)))
    ...
    // 1.0F,
    // 2.0F)))).when(BonusLevelTableCondition.bonusLevelFlatChance(Enchantments.BLOCK_FORTUNE,
    ...
    // {
    // return
    // LootTable.lootTable().withPool(LootPool.lootPool().setRolls(ConstantIntValue.exactly(1)).when(
    ...
    //
    // final List<ItemFossil> drops =
    // Lists.newArrayList(ItemGenerator.fossils.values());
    // LootPool.Builder fossilPool =
    // LootPool.lootPool().setRolls(ConstantIntValue.exactly(1)).add(LootItem.lootTableItem(
    ...
    // this.add(PokecubeItems.FOSSILSTONE.get(),
    // LootTable.lootTable().withPool(fossilPool));
    ...
    // this.add(fruit, BlockLoot.createSingleItemTable(item,
    // RandomValueBounds.between(1, 3)));
    // }
    // }
    //
    // @Override
    ...
    // {
    // if (map.put(location, builder.setParamSet(pair.getSecond()).build()) !=
    // null)
    ...
    // {
    // return pathIn.resolve("data/" + id.getNamespace() + "/loot_tables/" +
    // id.getPath() + ".json");
    // }
    //
    // @Override
    // protected List<Pair<Supplier<Consumer<BiConsumer<ResourceLocation,
    // LootTable.Builder>>>, LootContextParamSet>> getTables()
    // {
    // return ImmutableList.of(Pair.of(BlockLoot::new,
