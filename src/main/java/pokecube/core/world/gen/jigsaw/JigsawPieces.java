...
public class JigsawPieces
{
    ...
    public static void initPool(final TemplateManager manager, final JigSawPool pool)
    {
        JigsawPieces.initPool(manager, pool, (p, t) ->
        { // Do nothing by default.
        }, ImmutableList.of(PokecubeStructureProcessor.PROCESSOR, JigsawPieces.RULES));
    }

    public static void initPool(final TemplateManager manager, final JigSawPool pool,
            final BiConsumer<SingleJigsawPiece, JsonObject> proc, final List<StructureProcessor> rules)
    {
        JigsawPieces.pools.put(pool.name, pool);
        JigsawPieces.patterns.put(pool.name, JigsawPieces.registerPart(manager, pool, proc, rules));
    }

    private static JigsawPatternCustom registerPart(final TemplateManager manager, final JigSawPool part,
            final BiConsumer<SingleJigsawPiece, JsonObject> proc, final List<StructureProcessor> rules)
    {
        ...
        for (String option : part.options)
        {
            ...
            else parts.add(Pair.of(new SingleJigsawPiece(manager.getTemplateDefaulted(new ResourceLocation(option))),
                    second));
        }
    ...
    // {
    // public static IJigsawDeserializer<?> PIECETYPE =
    // IJigsawDeserializer.register("pokecube:sop",
    ...
    // final List<StructureProcessor> processors, final PlacementBehaviour type,
    ...
    // map.put(ops.createString("pool"),
    // ops.createString(PokedexEntryLoader.gson.toJson(this.pool)));
    // map.put(ops.createString("subbiome"), ops.createString(this.subbiome));
    // map.put(ops.createString("flag"), ops.createString(this.flag));
    // map.put(ops.createString("location"),
    // ops.createString(this.location.toString()));
    // map.put(ops.createString("ignoreAir"),
    // ops.createBoolean(this.ignoreAir));
    // map.put(ops.createString("dy"), ops.createInt(this.dy));
    // map.put(ops.createString("offset"), ops.createInt(this.offset));
    // map.put(ops.createString("processors"),
    // ops.createList(this.processors.stream().map((proc) ->
    // {
    ...
    // try
    // {
    // return new SpawnBiomeMatcher(PokedexEntryLoader.gson.fromJson(rule,
    // SpawnRule.class));
    // }
    // catch (final Exception e)
    // {
    ...
    // }
    //
    // public SingleOffsetPiece process(final JsonObject thing, final
    // BiConsumer<SingleOffsetPiece, JsonObject> proc)
    // {
    // if (thing != null) try
    // {
    // if (thing.has("flag")) this.flag = thing.get("flag").getAsString();
    // if (thing.has("ignoreAir")) this.ignoreAir =
    // thing.get("ignoreAir").getAsBoolean();
    // if (thing.has("subbiome")) this.subbiome =
    // thing.get("subbiome").getAsString();
    ...
    //
    // @Override
    // public PlacementSettings createPlacementSettings(final Rotation
    // p_214860_1_,
    ...
    // if (this.pool.filler)
    // this.last_used.addProcessor(FillerProcessor.PROCESSOR);
    // if (this.ignoreAir)
    // this.last_used.addProcessor(BlockIgnoreStructureProcessor.AIR_AND_STRUCTURE_BLOCK);
    // else
    // this.last_used.addProcessor(BlockIgnoreStructureProcessor.STRUCTURE_BLOCK);
    ...
    //
    // @Override
    // public boolean place(final TemplateManager manager, final IWorld worldIn,
    // final ChunkGenerator<?> p_225575_3_,
    // final BlockPos pos, final Rotation rotation, final MutableBoundingBox
    // box, final Random rand)
    // {
    //
    // final Template template = this.getTemplate(manager);
    // final PlacementSettings placementsettings =
    // this.createPlacementSettings(rotation, box);
    //
    // if (!template.addBlocksToWorld(worldIn, pos, placementsettings, 18))
    // return false;
    // else
    // {
    // final StructureEvent.BuildStructure event = new
    // StructureEvent.BuildStructure(box, worldIn,
    ...
    // for (final Template.BlockInfo template$blockinfo :
    // template.func_215381_a(pos, placementsettings,
    // Blocks.STRUCTURE_BLOCK))
    // if (template$blockinfo.nbt != null)
    // {
    // final StructureMode structuremode =
    // StructureMode.valueOf(template$blockinfo.nbt.getString(
    // "mode"));
    // if (structuremode == StructureMode.DATA)
    // this.handleDataMarker(template$blockinfo.nbt.getString(
    // "metadata"), template$blockinfo.pos, worldIn, rand, box);
    // }
    // // Back to the stuff that the superclass does.
    // for (final Template.BlockInfo template$blockinfo :
    // Template.processBlockInfos(template, worldIn, pos,
    // placementsettings, this.getDataMarkers(manager, pos, rotation, false)))
    // this.handleDataMarker(worldIn, template$blockinfo, pos, rotation, rand,
    // box);
    // return true;
    // }
    // }
    //
    // protected void handleDataMarker(String function, final BlockPos pos,
    // final IWorld worldIn, final Random rand,
    // final MutableBoundingBox sbb)
    // {
    // this.isSpawn = this.isSpawn &&
    // !PokecubeSerializer.getInstance().hasPlacedProf();
    ...
    // ResourceLocation(function.replaceFirst("pokecube:chest:", ""));
    // if (sbb.isVecInside(blockpos))
    // LockableLootTileEntity.setLootTable(worldIn, rand, blockpos, key);
    // }
    ...
    // worldIn.setBlockState(pos, Blocks.AIR.getDefaultState(), 2);
    // final ResourceLocation key = new
    // ResourceLocation(function.replaceFirst("Chest ", ""));
    // if (sbb.isVecInside(blockpos))
    // LockableLootTileEntity.setLootTable(worldIn, rand, blockpos, key);
    // }
    ...
    // final Event event = new StructureEvent.ReadTag(function.trim(), pos,
    // worldIn, rand, sbb);
    // MinecraftForge.EVENT_BUS.post(event);
    // if (event.getResult() == Result.ALLOW) JigsawPieces.apply(pos, (World)
    // worldIn);
    // }
    // }
    // }
