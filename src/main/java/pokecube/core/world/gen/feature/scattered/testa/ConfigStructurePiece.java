...
{
    public static final BlockIgnoreStructureProcessor IGNORELIST = new BlockIgnoreStructureProcessor(
            ImmutableList.of(Blocks.STRUCTURE_BLOCK, Blocks.JIGSAW));
    ...
    private void init(final TemplateManager manager)
    {
        ...
        final PlacementSettings placementsettings = new PlacementSettings().setIgnoreEntities(false)
                .setRotation(this.rot).setCenterOffset(this.centreOffset).setMirror(Mirror.NONE)
                .addProcessor(ConfigStructurePiece.IGNORELIST).addProcessor(PokecubeStructureProcessor.PROCESSOR);
        this.setup(this.to_build, this.templatePosition, placementsettings);
    }

    @Override
    public boolean func_225577_a_(final IWorld worldIn, final ChunkGenerator<?> p_225577_2_, final Random randomIn,
            final MutableBoundingBox structureBoundingBoxIn, final ChunkPos p_74875_4_)
    {
        try
        {
            if (!this.set)
            {
                ...
                double sum = 0;
                double count = 0;
                final BlockPos blockpos = this.templatePosition.add(this.to_build.getSize().getX() - 1, 0,
                        this.to_build.getSize().getZ() - 1);
                if (this.struct.surface)
                {
                    for (final BlockPos blockpos1 : BlockPos.getAllInBoxMutable(this.templatePosition, blockpos))
                    {
                        final int k = worldIn.getHeight(Heightmap.Type.OCEAN_FLOOR_WG, blockpos1.getX(),
                                blockpos1.getZ());
                        min = Math.min(min, k);
                        max = Math.max(max, k);
                        sum += k;
                        ...
                        inWater = inWater || worldIn.getBlockState(blockpos1.up()).getMaterial() == Material.WATER
                                || worldIn.getBlockState(blockpos1.down()).getMaterial() == Material.WATER
                                || worldIn.getBlockState(blockpos1).getMaterial() == Material.WATER
                                || k < worldIn.getSeaLevel() + this.floor;
                    }
                    ...
                    // height.
                    if (this.entrance != null) i = worldIn.getHeight(Heightmap.Type.OCEAN_FLOOR_WG,
                            this.entrance.getX(), this.entrance.getZ());
                }
                i += this.struct.offset + this.floor;
                if (World.isYOutOfBounds(i + dy))
                {
                    PokecubeCore.LOGGER
                    .error("Attempted to place a structure out of build bounds! " + this.templatePosition);
                    return false;
                }
            ...
            final boolean built = super.func_225577_a_(worldIn, p_225577_2_, randomIn, structureBoundingBoxIn,
                    p_74875_4_);
            terrain:
                if (built)
                {
