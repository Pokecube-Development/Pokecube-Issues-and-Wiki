...
public class Health
{
    ...
    public static void renderHealthBar(final LivingEntity passedEntity, final MatrixStack mat,
            final IRenderTypeBuffer buf, final float partialTicks,
            final Entity viewPoint)
    {
            ...
            processing:
            {
                ...
                final boolean captureOrHatch = StatsCollector.getCaptured(name_entry, Minecraft
                        .getInstance().player) > 0 || StatsCollector.getHatched(name_entry, Minecraft
                                ...
                                if (!stack.isEmpty() && config.showHeldItem) //                                    Health.renderIcon(off, 0, stack, 16, 16);
                                off -= 16;
    ...
    public static void renderIcon(final IVertexBuilder buffer, final int vertexX, final int vertexY,
            final ItemStack stack, final int intU,
            final int intV)
    {
        try
        {
            // FIXME rendering held item icons
            // final IBakedModel iBakedModel =
            // Minecraft.getInstance().getItemRenderer().getItemModelMesher().getItemModel(
            // stack);
            // final TextureAtlasSprite textureAtlasSprite =
            // Minecraft.getInstance().getTextureManager().getSprite(iBakedModel
            // .getParticleTexture(EmptyModelData.INSTANCE).getName());
            // buffer.begin(7, DefaultVertexFormats.POSITION_TEX);
            // buffer.pos(vertexX, vertexY + intV,
            // 0.0D).tex(textureAtlasSprite.getMinU(),
            // textureAtlasSprite.getMaxV())
            // .endVertex();
            // buffer.pos(vertexX + intU, vertexY + intV,
            // 0.0D).tex(textureAtlasSprite.getMaxU(), textureAtlasSprite
            // .getMaxV()).endVertex();
            // buffer.pos(vertexX + intU, vertexY,
            // 0.0D).tex(textureAtlasSprite.getMaxU(),
            // textureAtlasSprite.getMinV())
            // .endVertex();
            // buffer.pos(vertexX, vertexY,
            // 0.0D).tex(textureAtlasSprite.getMinU(),
            // textureAtlasSprite.getMinV())
            // .endVertex();
            // tessellator.draw();
