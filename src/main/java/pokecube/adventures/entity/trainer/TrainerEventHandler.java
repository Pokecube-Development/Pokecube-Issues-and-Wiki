...
public class TrainerEventHandler
{
    ...
    private static void attach_guard(final AttachCapabilitiesEvent<Entity> event)
    {
        ...
        if (event.getObject() instanceof VillagerEntity || event.getObject() instanceof EntityTrainer)
            event.addCapability(TrainerEventHandler.GUARDCAP, new Provider());
    }
    ...
    private static void attach_pokemobs(final AttachCapabilitiesEvent<Entity> event)
    {
        ...
        if (mob instanceof TrainerBase) event.addCapability(EventsHandler.TEXTURECAP,
                new NPCCap<>(mob, e -> mobs.getType().getTexture(e), e -> mobs.getGender() == 2));
    ...
    private static DataSync getData(final AttachCapabilitiesEvent<Entity> event)
    {
        for (final ICapabilityProvider provider : event.getCapabilities().values())
            if (provider.getCapability(SyncHandler.CAP).isPresent())
                return provider.getCapability(SyncHandler.CAP).orElse(null);
        return null;
    }
    ...
    public static void livingHurtEvent(final LivingHurtEvent evt)
    {
        ...
        if (evt.getEntity() instanceof INPC && !Config.instance.pokemobsHarmNPCs
        ...
        if (evt.getSource().getTrueSource() instanceof LivingEntity)
        {
            if (messages != null)
            {
                messages.sendMessage(MessageState.HURT, evt.getSource().getTrueSource(),
                        evt.getEntity().getDisplayName(), evt.getSource().getTrueSource().getDisplayName());
    ...
    public static void livingSetTargetEvent(final LivingSetAttackTargetEvent evt)
    {
        ...
        if (pokemobHolder != null && pokemobHolder.getTarget() == null)
            pokemobHolder.setTarget((LivingEntity) evt.getEntity());
    }
    ...
    public static void onJoinWorld(final EntityJoinWorldEvent event)
    {
        ...
        if (npc instanceof MobEntity && npc.isServerWorld())
        {
            ...
            if (Config.instance.trainersBattleEachOther)
                ais.add(new AIFindTarget(npc, 0.01f, npc.getClass()).setPriority(20));
        ...
        final int level = SpawnHandler.getSpawnLevel(npc.getEntityWorld(), Vector3.getNewVector().set(npc),
                Database.getEntry(1));
