...
public class DistorticPlankBarrelTile extends LockableLootTileEntity {
	private NonNullList<ItemStack> items = NonNullList.withSize(27, ItemStack.EMPTY);
	private int openCount;
	
	private DistorticPlankBarrelTile(TileEntityType<?> tileEntityType) {
		super(tileEntityType);
	}
	
	public DistorticPlankBarrelTile() {
		this(TileEntityInit.DISTORTIC_PLANK_BARREL_TILE.get());
	}
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + Reference.ID + ".distortic_barrel");
	}
	...
	public void startOpen(PlayerEntity player) {
		if (!player.isSpectator()) {
			...
			boolean flag = blockstate.getValue(DistorticPlankBarrel.OPEN);
			if (!flag) {
	...
	public void recheckOpen() {
		...
		if (this.openCount > 0) {
			...
			BlockState blockstate = this.getBlockState();
			if (!blockstate.is(BlockInit.DISTORTIC_BARREL.get())) {
				this.setRemoved();
				return;
			}
	...
	private void updateBlockState(BlockState state, boolean update) {
		this.level.setBlock(this.getBlockPos(), state.setValue(DistorticPlankBarrel.OPEN, Boolean.valueOf(update)), 3);
	}
	
	private void playSound(BlockState state, SoundEvent sound) {
		Vector3i vector3i = state.getValue(DistorticPlankBarrel.FACING).getNormal();
