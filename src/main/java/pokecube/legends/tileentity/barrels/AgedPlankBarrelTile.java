...
public class AgedPlankBarrelTile extends LockableLootTileEntity {
	private NonNullList<ItemStack> items = NonNullList.withSize(27, ItemStack.EMPTY);
	private int openCount;
	
	private AgedPlankBarrelTile(TileEntityType<?> tileEntityType) {
		super(tileEntityType);
	}
	
	public AgedPlankBarrelTile() {
		this(TileEntityInit.AGED_PLANK_BARREL_TILE.get());
	}
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + Reference.ID + ".aged_barrel");
	}
	...
	public void startOpen(PlayerEntity player) {
		if (!player.isSpectator()) {
			...
			boolean flag = blockstate.getValue(AgedPlankBarrel.OPEN);
			if (!flag) {
	...
	public void recheckOpen() {
		...
		if (this.openCount > 0) {
			...
			BlockState blockstate = this.getBlockState();
			if (!blockstate.is(BlockInit.AGED_BARREL.get())) {
				this.setRemoved();
				return;
			}
	...
	private void updateBlockState(BlockState state, boolean update) {
		this.level.setBlock(this.getBlockPos(), state.setValue(AgedPlankBarrel.OPEN, Boolean.valueOf(update)), 3);
	}
	
	private void playSound(BlockState state, SoundEvent sound) {
		Vector3i vector3i = state.getValue(AgedPlankBarrel.FACING).getNormal();
