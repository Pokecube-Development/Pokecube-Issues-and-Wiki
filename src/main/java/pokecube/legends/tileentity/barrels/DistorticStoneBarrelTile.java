...
public class DistorticStoneBarrelTile extends LockableLootTileEntity {
	private NonNullList<ItemStack> items = NonNullList.withSize(27, ItemStack.EMPTY);
	private int openCount;
	
	private DistorticStoneBarrelTile(TileEntityType<?> tileEntityType) {
		super(tileEntityType);
	}
	
	public DistorticStoneBarrelTile() {
		this(TileEntityInit.DISTORTIC_STONE_BARREL_TILE.get());
	}
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + Reference.ID + ".distortic_stone_barrel");
	}
	...
	public void startOpen(PlayerEntity player) {
		if (!player.isSpectator()) {
			...
			boolean flag = blockstate.getValue(DistorticStoneBarrel.OPEN);
			if (!flag) {
				this.playSound(blockstate, SoundEvents.NETHER_BRICKS_BREAK);
	...
	public void recheckOpen() {
		...
		if (this.openCount > 0) {
			...
			BlockState blockstate = this.getBlockState();
			if (!blockstate.is(BlockInit.DISTORTIC_STONE_BARREL.get())) {
				this.setRemoved();
				return;
			}
			...
			boolean flag = blockstate.getValue(DistorticStoneBarrel.OPEN);
			if (flag) {
				this.playSound(blockstate, SoundEvents.NETHER_BRICKS_PLACE);
	...
	private void updateBlockState(BlockState state, boolean update) {
		this.level.setBlock(this.getBlockPos(), state.setValue(DistorticStoneBarrel.OPEN, Boolean.valueOf(update)), 3);
	}
	
	private void playSound(BlockState state, SoundEvent sound) {
		Vector3i vector3i = state.getValue(DistorticStoneBarrel.FACING).getNormal();
