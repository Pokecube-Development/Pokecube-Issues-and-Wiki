...
public class ConcreteBarrelTile extends LockableLootTileEntity {
	private NonNullList<ItemStack> items = NonNullList.withSize(27, ItemStack.EMPTY);
	private int openCount;
	
	private ConcreteBarrelTile(TileEntityType<?> tileEntityType) {
		super(tileEntityType);
	}
	
	public ConcreteBarrelTile() {
		this(TileEntityInit.CONCRETE_PLANK_BARREL_TILE.get());
	}
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + Reference.ID + ".concrete_barrel");
	}
	...
	public void startOpen(PlayerEntity player) {
		if (!player.isSpectator()) {
			...
			boolean flag = blockstate.getValue(ConcretePlankBarrel.OPEN);
			if (!flag) {
				this.playSound(blockstate, SoundEvents.STONE_BREAK);
	...
	public void recheckOpen() {
		...
		if (this.openCount > 0) {
			...
			BlockState blockstate = this.getBlockState();
			if (!blockstate.is(BlockInit.CONCRETE_BARREL.get())) {
				this.setRemoved();
				return;
			}
			...
			boolean flag = blockstate.getValue(ConcretePlankBarrel.OPEN);
			if (flag) {
				this.playSound(blockstate, SoundEvents.STONE_PLACE);
	...
	private void updateBlockState(BlockState state, boolean update) {
		this.level.setBlock(this.getBlockPos(), state.setValue(ConcretePlankBarrel.OPEN, Boolean.valueOf(update)), 3);
	}
