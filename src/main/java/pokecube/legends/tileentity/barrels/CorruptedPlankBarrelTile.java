...
public class CorruptedPlankBarrelTile extends LockableLootTileEntity {
	private NonNullList<ItemStack> items = NonNullList.withSize(27, ItemStack.EMPTY);
	private int openCount;
	
	private CorruptedPlankBarrelTile(TileEntityType<?> tileEntityType) {
		super(tileEntityType);
	}
	
	public CorruptedPlankBarrelTile() {
		this(TileEntityInit.CORRUPTED_PLANK_BARREL_TILE.get());
	}
	...
	protected ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + Reference.ID + ".corrupted_barrel");
	}
	...
	public void startOpen(PlayerEntity player) {
		if (!player.isSpectator()) {
			...
			boolean flag = blockstate.getValue(CorruptedPlankBarrel.OPEN);
			if (!flag) {
	...
	public void recheckOpen() {
		...
		if (this.openCount > 0) {
			...
			BlockState blockstate = this.getBlockState();
			if (!blockstate.is(BlockInit.CORRUPTED_BARREL.get())) {
				this.setRemoved();
				return;
			}
	...
	private void updateBlockState(BlockState state, boolean update) {
		this.level.setBlock(this.getBlockPos(), state.setValue(CorruptedPlankBarrel.OPEN, Boolean.valueOf(update)), 3);
	}
	
	private void playSound(BlockState state, SoundEvent sound) {
		Vector3i vector3i = state.getValue(CorruptedPlankBarrel.FACING).getNormal();
