...
public class Regidrago extends AbstractRegiCondition
{
    public Regidrago()
    {
        super(BlockInit.GOLEM_STONE.get(), BlockInit.REGIDRAGO_CORE.get(), Blocks.OBSIDIAN);
    }
    ...
    public CanSpawn canSpawn(final Entity trainer, final Vector3 location, final boolean message)
    {
        ...
        if (!check)
        {
            if (message) this.sendLegendBuild(trainer, "Regidrago");
            return CanSpawn.NO;
        }
        return CanSpawn.YES;
    }
    ...
    {
        return Database.getEntry("regidrago");
    }
