...
{
    private static final ResourceLocation VALID_BLOCKS = new ResourceLocation(Reference.ID, "arceus_approved/regieleki");
    public Regieleki()
    {
    ...
    public CanSpawn canSpawn(SpawnContext context, final boolean message)
    {
        ...
        if (!check)
        {
            if (message) this.sendLegendBuild(context.player(), "Regieleki");
            return CanSpawn.NO;
        }
        return CanSpawn.YES;
    }
