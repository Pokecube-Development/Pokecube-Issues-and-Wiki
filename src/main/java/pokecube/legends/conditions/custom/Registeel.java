...
{
    private static final ResourceLocation VALID_BLOCKS = new ResourceLocation(Reference.ID, "arceus_approved/registeel");
    public Registeel()
    {
    ...
    public CanSpawn canSpawn(SpawnContext context, final boolean message)
    {
        ...
        if (!check)
        {
            if (message) this.sendLegendBuild(context.player(), "Registeel");
            return CanSpawn.NO;
        }
        return CanSpawn.YES;
    }
