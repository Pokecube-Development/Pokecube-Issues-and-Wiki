...
{		
	...
	public EffectBlockBase(final String name, final Material material, final MaterialColor color, final float hardness, final float resistance,
			final SoundType sound, final ToolType tool, final int harvestLevel, final boolean hadDrop, final MobEffect effects) 
	{
		super(name, material, color, hardness, resistance, sound, tool, harvestLevel, hadDrop, effects);
		this.effect = effects;
	}
	
	public EffectBlockBase(final Material material, final MaterialColor color, final float hardness, final float resistance,
			final SoundType sound, final ToolType tool, final int harvestLevel, final boolean hadDrop, final MobEffect effects) 
	{
    ...
    public static void applyEffects(final Entity entity, final MobEffect effects)
    {
        if (entity instanceof ServerPlayer) if (((Player) entity).getInventory().armor.get(3).getItem() != new ItemStack(ItemInit.ULTRA_HELMET.get(), 1).getItem() ||
            ...
            ((Player) entity).getInventory().armor.get(0).getItem() != new ItemStack(ItemInit.ULTRA_BOOTS.get(), 1).getItem()) ((LivingEntity) entity).addEffect(new MobEffectInstance(effects, 120, 1));
    }
}
