...
public class PinkDynaLeavesBlock extends DynaLeavesBlock implements IForgeShearable
{
    int particleSpawnSpeed;

    public PinkDynaLeavesBlock(int particleSpawnSpeed, final Properties properties)
    {
        super(properties);
    ...
    public void animateTick(final BlockState state, final Level world, final BlockPos pos, final RandomSource random)
    {
        ...
        if (random.nextInt(particleSpawnSpeed) == 0) {
            ...
            {
                ParticleUtils.spawnParticleBelow(world, pos, random, ParticleInit.DYNA_LEAF_PINK.get());
            }
        }
    }
}
