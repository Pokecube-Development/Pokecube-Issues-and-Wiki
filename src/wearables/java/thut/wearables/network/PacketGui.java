...
public class PacketGui extends Packet
{
    public static class WearableContext extends UseOnContext
    {
        ...
        {
            super(player.level(), player, InteractionHand.MAIN_HAND, heldItem, WearableContext.fromNBT(player, nbt));
        }
    }
    ...
    public void handleServer(final ServerPlayer player)
    {
        if (this.data.contains("S"))
        {
            ...
            if (!stack.isEmpty())
            {
                ...
                if (!event.isCanceled()) EnumWearable.interact(player, stack, slot, context);
            }
        }
        else if (this.data.contains("close"))
        {
            ...
            else
            {
                ...
                player.openMenu(provider, buf -> buf.writeInt(t.getId()));
            }
        }
        else
        {
            ...
            {
                final Entity mob = player.level().getEntity(this.data.getInt("w_open_target_"));
    ...
    private final static Type<Packet> TYPE = new Type<Packet>(ResourceLocation.parse("thut_wearables:gui_access"));
