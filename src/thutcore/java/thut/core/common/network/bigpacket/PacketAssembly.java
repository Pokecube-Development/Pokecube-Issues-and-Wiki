 ...
 */
public final class PacketAssembly<T extends BigPacket>
{
    ...
    public static <K extends BigPacket> PacketAssembly<K> registerAssembler(final Class<K> clazz,
    ...
    public static final Map<Class<? extends BigPacket>, PacketAssembly<?>> ASSEMBLERS = new HashMap<>();
    ...
    protected byte[] onRead(final CompoundTag tag)
    {
        final UUID id = tag.getUUID("id");
        final byte[] made = this.assemblePacket(id, tag);
        return made;
    }

    private List<CompoundTag> splitPacket(final UUID id, final byte[] data)
    {
        try
        {
     ...
     * when complete
     */
    private byte[] assemblePacket(final UUID id, final CompoundTag tags)
    {
