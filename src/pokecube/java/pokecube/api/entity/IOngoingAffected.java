...
public interface IOngoingAffected extends INBTSerializable<ListTag>
{
    public static interface IOngoingEffect extends INBTSerializable<CompoundTag>
    {
        ...
        @Override
        default void deserializeNBT(Provider access, final CompoundTag nbt)
        {
        ...
        @Override
        default CompoundTag serializeNBT(Provider access)
        {
    ...
    @Override
    default void deserializeNBT(Provider access, final ListTag nbt)
    {
        this.clearEffects();
        for (int i = 0; i < nbt.size(); i++)
        {
            ...
            final ResourceLocation loc = ResourceLocation.parse(key);
            ...
            if (effectClass != null) try
            {
                ...
                effect.deserializeNBT(access, value);
    ...
    @Override
    default ListTag serializeNBT(Provider access)
    {
        final ListTag list = new ListTag();
        for (final IOngoingEffect effect : this.getEffects()) if (effect.onSavePersistant())
        {
            final CompoundTag tag = effect.serializeNBT(access);
            if (tag != null)
            {
