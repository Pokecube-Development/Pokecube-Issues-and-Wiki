...
public class MiscItemHelper
{
    public static class CharcoalEffect extends BaseUseable
    {
        ...
        {
            if (pokemob == moveuse.getUser() && pre) if (moveuse.getMove().getType(pokemob) == CharcoalEffect.FIRE)
            {
                moveuse.pwr *= 1.2;
    ...
    public static final ResourceLocation USABLE = ResourceLocation.fromNamespaceAndPath(PokecubeMobs.MODID, "usables");

    static ItemStack CHARCOALSTACK;

    private static InteractionResultHolder<ItemStack> applyEVs(final byte[] evs, final ItemStack stack,
    ...
    private static InteractionResultHolder<ItemStack> feedVitamin(final ItemStack stack, final Entity entity)
    {
        ...
        if (pokemob != null)
        {
            if (ItemList.is(ResourceLocation.parse("pokecube:vit_hpup"), stack))
                return MiscItemHelper.applyEVs(new byte[]
                { 10, 0, 0, 0, 0, 0 }, stack, pokemob);
            if (ItemList.is(ResourceLocation.parse("pokecube:vit_protein"), stack))
                return MiscItemHelper.applyEVs(new byte[]
                { 0, 10, 0, 0, 0, 0 }, stack, pokemob);
            if (ItemList.is(ResourceLocation.parse("pokecube:vit_iron"), stack))
                return MiscItemHelper.applyEVs(new byte[]
                { 0, 0, 10, 0, 0, 0 }, stack, pokemob);
            if (ItemList.is(ResourceLocation.parse("pokecube:vit_calcium"), stack))
                return MiscItemHelper.applyEVs(new byte[]
                { 0, 0, 0, 10, 0, 0 }, stack, pokemob);
            if (ItemList.is(ResourceLocation.parse("pokecube:vit_zinc"), stack))
                return MiscItemHelper.applyEVs(new byte[]
                { 0, 10, 0, 0, 10, 0 }, stack, pokemob);
            if (ItemList.is(ResourceLocation.parse("pokecube:vit_carbos"), stack))
                return MiscItemHelper.applyEVs(new byte[]
                { 0, 0, 0, 0, 0, 10 }, stack, pokemob);
        }
        return new InteractionResultHolder<>(InteractionResult.FAIL, stack);
    }
    ...
    public static void init()
    {
        ...
        final VitaminEffect value = new VitaminEffect()
        {
            ...
            {
                final InteractionResultHolder<ItemStack> result = MiscItemHelper.feedVitamin(stack,
                        pokemob.getEntity());
        ...
        UsableItemEffects.REGISTRY.put(
                i -> MiscItemHelper.CHARCOALSTACK != null && i == MiscItemHelper.CHARCOALSTACK.getItem(),
                CharcoalEffect::new);
    }
}
