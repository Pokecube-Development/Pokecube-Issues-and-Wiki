...
public class TeraTypeGene implements Gene<TeraTypeGene.TeraType>
{
    ...
    protected static void checkHats()
    {
        if (SILLY_HATS.isEmpty())
        {
            ...
            for (PokeType type : PokeType.values())
            {
                ...
                HAT.set(DataComponents.DYED_COLOR, colourData);
                HAT.set(ThutBling.BLING_GEM_DATA, gemData);
                HAT.set(ThutBling.BLING_MODEL_DATA, modelData);
    ...
    @Override
    public void load(Provider provider, CompoundTag tag)
    {
        this.getValue().deserializeNBT(provider, tag);
    }
    ...
    {
        return this.getValue().serializeNBT(provider);
    }
