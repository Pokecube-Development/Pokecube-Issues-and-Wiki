...
public class ForgetTargetTask extends CombatTask
{
    ...
    protected void tick(final ServerLevel level, final Mob entity, final long gameTime)
    {
        ...
        if (!this.forgotten.isEmpty())
        {
            ...
            for (final UUID id : ids)
                if (!this.forgotten.get(id).isValid(level.getGameTime())) this.forgotten.remove(id);
        }
        ...
        if (mobB == null && target.getRemovalReason() == RemovalReason.DISCARDED)
        {
            deAgro = true;
        }
        ...
        if (!deAgro)
        {
            ...
            if (!pokemob.getCombatState(CombatStates.BATTLING))
            {
            ...
            if (target.getUUID().equals(pokemob.getOwnerId()))
            {
            ...
            final boolean tame = pokemob.getGeneralState(GeneralStates.TAMED);
            ...
            if (tame)
            {
                ...
                final boolean stayOrGuard =
                        pokemob.getCombatState(CombatStates.GUARDING) || pokemob.getGeneralState(GeneralStates.STAYING);
                if (owner != null && !stayOrGuard && owner.distanceTo(entity) > PokecubeCore.getConfig().chaseDistance)
                {
                ...
                if (!PokecubeCore.getConfig().teamsBattleEachOther && TeamManager.sameTeam(target, entity)
            ...
            if (BrainUtils.canSee(entity, target)) this.ticksSinceSeen = 0;
            ...
            distance:
            if (entity.distanceTo(target) > PokecubeCore.getConfig().chaseDistance)
            {
                // Check if we are owned
                boolean owned = pokemob.getOwnerId() != null && !pokemob.isPlayerOwned();
    ...
    {
        final LivingEntity target = this.getAttackTarget(entityIn);
        // Only run if we have a combat target
        return target != null;
    }

    private void doDeAgro(IPokemob pokemob, boolean exitBattle)
    {
