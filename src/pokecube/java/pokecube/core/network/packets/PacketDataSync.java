...
public class PacketDataSync extends Packet
{
    ...
    @Override
    public void handleClient(Player playerIn)
    {
        final UUID id = this.data.hasUUID("uuid") ? this.data.getUUID("uuid") : null;
        final Player player = id == null ? playerIn : PokecubeCore.proxy.getPlayer(id);
        if (player == null) return;
        final PlayerDataManager manager = PlayerDataHandler.getInstance().getPlayerData(player);
        manager.getData(this.data.getString("type")).readFromNBT(player.registryAccess(),
                this.data.getCompound("data"));
    }
