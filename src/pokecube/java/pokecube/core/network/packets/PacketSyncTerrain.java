...
public class PacketSyncTerrain extends Packet
{
    ...
    {
        final TerrainSegment t = TerrainManager.getInstance().getTerrain(player.level(), this.x * 16, this.y * 16,
                this.z * 16);
    ...
    {
        for (final long[] effect : array) for (int s = 0; s < 2; s++) buf.writeLong(effect[s]);
    }

    private void readLongArray(final long[][] array, final FriendlyByteBuf buf)
    {
        for (final long[] effect : array) for (int s = 0; s < 2; s++) effect[s] = buf.readLong();
    }

    private final static Type<Packet> TYPE = new Type<Packet>(ResourceLocation.parse("pokecube:sync_terrain"));
