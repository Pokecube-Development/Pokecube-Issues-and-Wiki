...
public class Kill
{
     ...
     */
    public static class KillCommandEvent extends LivingEvent implements ICancellableEvent
    {
            ...
            throws CommandSyntaxException
    {
        ...
        for (final Entity o : mobs)
        {
            ...
            if (e != null && !e.getEntity().isInvulnerable())
            {
                try
                {
                    ...
                    if (PokecubeAPI.POKEMOB_BUS.post(event).isCanceled()) continue;
                    e.onRecall();
                    count1++;
                }
                catch (Exception e1)
        ...
        source.sendSuccess(() -> Component.translatableEscape("pokecube.command." + (cull ? "cull" : "kill"), finalCount), true);
        return 0;
    }
