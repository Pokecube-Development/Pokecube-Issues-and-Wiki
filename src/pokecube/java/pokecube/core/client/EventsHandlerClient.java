...
public class EventsHandlerClient
{
    ...
    private static void onPlayerTick(final PlayerTickEvent.Post event)
    {
                ...
                && event.getEntity() == Minecraft.getInstance().player)
        {
            ...
            if (pokemob != null && e.getControllingPassenger() == event.getEntity())
            {
        ...
        if (lastSetTime < now)
        {
            ...
            hovorTarget = Tools.getPointedEntity(event.getEntity(), 32, selector, 1);
    ...
    public static ResourceLocation GUI_LAYER = ResourceLocation.parse("pokecube:hotbar_icon_overlay");
    ...
    {
        var contents = PokemobCaps.getPokemobIn(itemStack, world);
        if (contents == null) return null;
        return contents.pokemob();
    }
