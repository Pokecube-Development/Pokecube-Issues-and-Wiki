...
public class PokemobInfoPage extends PageWithSubPages<PokeInfoPage>
{
    ...
    public boolean keyPressed(final int keyCode, final int b, final int c)
    {
        if (this.searchBox.isFocused())
            if (keyCode == GLFW.GLFW_KEY_RIGHT && this.searchBox.getCursorPosition() == this.searchBox.getValue().length())
        {
    ...
    public void postPageDraw(final GuiGraphics graphics, final int mouseX, final int mouseY, final float partialTicks)
    {
        ...
        if (this.pokemob != null)
        {
            ...
            if (mx > -30 && mx < -30 + 50) if (my > 25 && my < 25 + 70)
            {
                ...
                graphics.fill(x + mx - 2, y + my - 2,
                        x + mx + box + 2, y + my + dy * text.size() + 2, 0xD92E0A65);
                for (final String s : text)
                {
                    graphics.fill(x + mx - 1, y + my - 1,
                            x + mx + box + 1, y + my + dy + 1, 0xD91E0F1E);

                    if (this.pokemob.isShiny()) graphics.drawString(this.font, s, x + mx + 1, y + my + 1, 0xFFFFBB6F);
                    else graphics.drawString(this.font, s, x + mx + 1, y + my + 1, 0xFFFFFFFF);
                    my += dy;
                }
    ...
    public void prePageDraw(final GuiGraphics graphics, final int mouseX, final int mouseY, final float partialTicks)
    {
        ...
        boolean drawLevel = this.watch.pokemob != null && this.watch.pokemob.getEntity().isAddedToLevel();
        ...
        if (this.pokemob != null)
        {
            ...
            dx = -77;
            dy = 32;
            // Only draw the lvl if it is a real mob, otherwise it will just say Lvl 1
