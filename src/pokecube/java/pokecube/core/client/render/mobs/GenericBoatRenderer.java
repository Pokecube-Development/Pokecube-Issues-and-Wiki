...
public class GenericBoatRenderer extends EntityRenderer<GenericBoat>
{
    ...
    public GenericBoatRenderer(EntityRendererProvider.Context context, boolean chest)
    {
        ...
        GenericBoat.getTypes().forEach(type -> {
            ...
            if (chest) boatResources.put(type.name(),
                    Pair.of(ResourceLocation.fromNamespaceAndPath(modid, "textures/entity/chest_boat/" + type.name() + ".png"),
                            new BoatModel(context.bakeLayer(createBoatModelName(modid, type, chest)))));
            else boatResources.put(type.name(),
                    Pair.of(ResourceLocation.fromNamespaceAndPath(modid, "textures/entity/boat/" + type.name() + ".png"),
                            new BoatModel(context.bakeLayer(createBoatModelName(modid, type, chest)))));
        });
    ...
    {
        if (chest)
            return new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(modid, "chest_boat/" + type.name()), "main");
        else return new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(modid, "boat/" + type.name()), "main");
    }
            ...
            MultiBufferSource source, int i)
    {
        ...
        if (f > 0.0F)
        {
            matricStack.mulPose(Axis.XP.rotationDegrees(Mth.sin(f) * f * f1 / 10.0F * boat.getHurtDir()));
        }
        ...
        boatmodel.renderToBuffer(matricStack, vertexconsumer, i, OverlayTexture.NO_OVERLAY);
