...
public abstract class PokemobSaves extends PokemobOwned implements TagNames
{
    ...
    public void deserializeNBT(Provider provider, final CompoundTag tag)
    {
        ...
        if (!statsTag.isEmpty())
        {
            ...
            this.params.HAPPYDW.set(statsTag.getInt(TagNames.HAPPY));
        }
        // Read moves tag
        if (!movesTag.isEmpty())
        {
        ...
        if (!inventoryTag.isEmpty())
        {
            ...
            for (int i = 0; i < ListNBT.size(); ++i)
            {
                ...
                if (j < this.getInventory().getContainerSize())
                    this.getInventory().setItem(j, ItemStack.parseOptional(provider, CompoundNBT1));
            }
        }
        // Read Breeding tag
        ...
        if (!visualsTag.isEmpty())
        {
            ...
            if (visualsTag.contains(TagNames.POKECUBE))
            {
                ...
                this.setPokecube(ItemStack.parseOptional(provider, pokecubeTag));
            }
    ...
    public CompoundTag serializeNBT(Provider provider)
    {
        ...
        statsTag.putInt(TagNames.HAPPY, this.params.HAPPYDW.get());
        ...
        for (int i = 0; i < this.getInventory().getContainerSize(); ++i)
        {
            ...
            if (!itemstack.isEmpty())
            {
                ...
                ListNBT.add(itemstack.save(provider, CompoundNBT1));
            }
        }
        ...
        {
            visualsTag.put(TagNames.POKECUBE, this.getPokecube().save(provider, new CompoundTag()));
        }
        // Misc AI
