...
public abstract class PokemobSexed extends PokemobSaves implements IBreedingMob
{
    ...
    public boolean canMate(final AgeableMob otherAnimal)
    {
        ...
        if (otherMob != null)
        {
            ...
            if (PokecubeAPI.POKEMOB_BUS.post(new EggEvent.CanBreed(this.getEntity(), otherAnimal)).isCanceled())
                return false;
            ...
            // Anything else will mate
            // with a transformer.
            if (transforms && otherTransforms) return false;
            else return transforms || otherTransforms;
        }
        return false;
    }
    ...
    public void lay(final IPokemob male)
    {
        ...
        if (pos.isClearOfBlocks(this.getEntity().level()))
        {
            ...
            if (!event.isCanceled())
            {
                ...
                if (player != null) Triggers.BREEDPOKEMOB.get().trigger(player, this, male);
                this.egg = eggItem;
    ...
    public void resetLoveStatus()
    {
        ...
        if (this.getEntity() instanceof AgeableMob mob) BrainUtils.setMateTarget(mob, null);
