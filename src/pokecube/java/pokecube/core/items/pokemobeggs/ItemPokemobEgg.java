...
public class ItemPokemobEgg extends Item
{
    ...
    public static ItemStack getEggStack(final PokedexEntry entry, Level level)
    {
        ...
        Mob pokemob = PokecubeCore.createPokemob(entry, level);
        eggItemStack.set(PokemobCaps.POKEEGG_DATA, new EggInfo());
        eggItemStack.set(DefaultGenetics.GENE_STORE, new GeneHolder(pokemob, pokemob.registryAccess()));
        return eggItemStack;
    }

    public static PokedexEntry getEntry(final ItemStack stack, Provider access)
    {
        IMobGenetics _genes = GeneticsManager.getGenes(stack, access);
        genes:
        if (_genes != null)
        {
...
//        nbt.putString("motherId", mother.getEntity().getStringUUID());
    ...
    private static LivingEntity imprintOwner(final IPokemob mob)
    {
        ...
        final IOwnable ownable = ThutCaps.getOwnable(owner);
    ...
    private static void initPokemobGenetics(final IPokemob mob, final ItemStack stack, final boolean imprint)
    {
        ...
        final IMobGenetics eggs = GeneticsManager.getGenes(stack, mob.getEntity().registryAccess());
    ...
    public static void initStack(final Entity mother, final IPokemob father, final ItemStack stack)
    {
        ...
        if (mob != null && father != null) ItemPokemobEgg.getGenetics(mob, father, stack, mother.registryAccess());
    }
    ...
    public boolean dropEgg(final Level world, final ItemStack stack, final Vector3 location, final Entity placer)
    {
        ...
        final ItemStack eggItemStack = stack.copyWithCount(1);
