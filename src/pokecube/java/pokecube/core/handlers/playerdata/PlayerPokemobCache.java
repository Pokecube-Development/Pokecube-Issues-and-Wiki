...
public class PlayerPokemobCache extends PlayerData
{
    ...
    private static void UpdateCacheImpl(final ItemStack stack, final boolean pc, final boolean deleted)
    {
        ...
        final PlayerPokemobCache cache = PlayerDataHandler.getInstance().getPlayerData(access(), owner)
    ...
    private static void Remove(final UUID uuid, final ItemStack stack)
    {
        ...
        PlayerDataHandler.getInstance().save(access(), owner, cache.getIdentifier());
    }
    ...
    public void addPokemob(final String owner, final ItemStack stack, boolean pc, final boolean deleted)
    {
        ...
        PlayerDataHandler.getInstance().save(access(), owner, this.getIdentifier());
    }
    ...
    @Override
    public void readFromNBT(Provider access, final CompoundTag tag)
    {
        this.cache.clear();
        this._in_pc_.clear();
        this._dead_.clear();
        if (tag.contains("data"))
        {
            ...
            for (int i = 0; i < list.size(); i++)
            {
                ...
                Integer id = -1;
                final ItemStack stack = ItemStack.parseOptional(access, var);
                if (var.contains("uid")) id = var.getInt("uid");
                else id = PokecubeManager.getUID(stack, level());
                if (id != -1)
                {
    ...
    @Override
    public void writeToNBT(Provider access, final CompoundTag tag)
    {
