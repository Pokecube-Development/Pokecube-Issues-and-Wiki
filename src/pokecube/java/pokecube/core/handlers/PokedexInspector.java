...
public class PokedexInspector
{
    ...
    public static boolean inspect(final Player player, final boolean reward)
    {
        ...
        if (evt.isCanceled())
        {
            ...
            PlayerDataHandler.getInstance().save(player.registryAccess(), uuid);
        }
    ...
    {
        final String uuid = evt.getEntity().getStringUUID();
        final PokecubePlayerCustomData data = PlayerDataHandler.getInstance()
                .getPlayerData(evt.getEntity().registryAccess(), uuid).getData(PokecubePlayerCustomData.class);
        boolean done = false;
