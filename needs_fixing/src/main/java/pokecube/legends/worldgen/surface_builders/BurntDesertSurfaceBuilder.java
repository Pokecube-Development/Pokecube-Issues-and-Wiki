...
public class BurntDesertSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderBaseConfiguration>
{
    public BurntDesertSurfaceBuilder(Codec<SurfaceBuilderBaseConfiguration> config)
    {
        super(config);
    }
            ...
            for(int l = startHeight; l >= num; --l)
            {
                ...
                } else if (blockstate4.is(block.getBlock()))
                {
                    ...
                    } else if (k > 0)
                    {
                        ...
                        if (k == 0 && blockstate3.is(BlockInit.BLACKENED_SAND.get()) && i > 1)
                        {
                            k = random.nextInt(4) + Math.max(0, l - seaLevel);
                            blockstate3 = BlockInit.BLACKENED_SANDSTONE.get().defaultBlockState();
                        }
                    }
                }
            }
