...
Pokecubes are the items used to capture Pokemobs. They are primarily governed by the [IPokecube](../src/pokecube/java/pokecube/api/items/IPokecube.java) interface.
[IPokecube](../src/pokecube/java/pokecube/api/items/IPokecube.java)
[IPokecube](../src/pokecube/java/pokecube/api/items/IPokecube.java)s are used to determine capture rates, as well as to provide the functions needed to throw pokecubes at things.
...
The default [IPokecube](../src/pokecube/java/pokecube/api/items/IPokecube.java) implementation is the [Pokecube](../src/pokecube/java/pokecube/core/items/pokecubes/Pokecube.java) item class. It provides the `throwPokecube` and `throwPokecubeAt` functions, which produce a [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) to throw. Capture itself is then handled by the [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java).

`throwPokecube` and `throwPokecubeAt` are presently called in 3 locations: 

1. When the player finishes "using" the [Pokecube](../src/pokecube/java/pokecube/core/items/pokecubes/Pokecube.java)
2. When a dispenser fires a [Pokecube](../src/pokecube/java/pokecube/core/items/pokecubes/Pokecube.java)
3. When an NPC attempts to [capture](npcs.md#trainer-combat-ai) a pokemob

## Capturing Pokemobs

Capturing pokemobs occurs when an [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) who's item does not contain an entity collides with a wild entity. This is done via `CaptureManager::captureAttempt`, which does the following:
...
3.  Fires `CaptureEvent.Pre` on the `PokecubeAPI.POKEMOB_BUS`
    -  If the entity is a [IPokemob](../src/pokecube/java/pokecube/api/entity/pokemob/IPokemob.java), then does as follows:
    ...
    -   If not a [IPokemob](../src/pokecube/java/pokecube/api/entity/pokemob/IPokemob.java):
...
If a capture was attempted, the [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) will attempt to shake 4 times, the number of shakes was determined during the `Tools::computeCatchRate` or `Tools::getCatchRate` steps above.

If it is successful, `CaptureManager::captureSucceed` is called. Here the captured mob is tamed by the thrower if the mob is tameable, and marked as added to the [Pokecube](../src/pokecube/java/pokecube/core/items/pokecubes/Pokecube.java). If the mob is an [IPokemob](../src/pokecube/java/pokecube/api/entity/pokemob/IPokemob.java), the happiness is set accordingly.
...
Releasing of an entity from a [Pokecube](../src/pokecube/java/pokecube/core/items/pokecubes/Pokecube.java) is handled via `SendOutManager::sendOut`. This re-creates the entity from the stored data inside the [Pokecube](../src/pokecube/java/pokecube/core/items/pokecubes/Pokecube.java), and then attempts to add it to the world. Here is checks the following:
...
-   Mob added to level
-   If a [IPokemob](../src/pokecube/java/pokecube/api/entity/pokemob/IPokemob.java):
...
2.  When a [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) is filled and collides with a block, and is marked to release on collision (sneaking while tossing does not mark as such)
3.  When a [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) is filled and collides with a mob
4.  When a [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) is filled and punched or interacted with, while not sneaking
5.  If a [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) is marked to `autoRelease` and times out

In the cases where the release occurs due to collision with a mob, a [Battle](../src/pokecube/java/pokecube/api/moves/Battle.java) is initiated.

## Loot Pokecubes

An [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) can also be marked as loot via the `isLoot` field (alternately by setting `isLoot` in the nbt and reloading it). If marked as such, then the interaction behaviour changes to giving items. These can either be set via the `lootTable` parameter as a vanilla `LootTable`, or manually as items via `LootEntry` objects in the `loot` nbt list. 

If a [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) is loot, then when interacted with, it gives the item, and then disappears for a configurable `resetTime`. Once collected, the [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java) will remain invisible to the player until the `resetTime` has passed, and the player leaves and re-enters the area (ie client load/unload of the [EntityPokecube](../src/pokecube/java/pokecube/core/entity/pokecubes/EntityPokecube.java))